...
// ─────────────────────────────────────────────────────────────────────────────
// Análisis de patrón alimenticio por rango de fechas
// ─────────────────────────────────────────────────────────────────────────────

interface ConsumoParaAnalisis {
  nombre?: string;
  tipoComida?: string;
...
interface AnalizarPatronRequest {
  consumos: ConsumoParaAnalisis[];
  rangoFechas: string;
  perfilNutricional?: PerfilNutricional;
}

interface AnalizarPatronResponse {
  analisis: string;
  resumen: {
...
function extraerCalificacionGeneral(raw: string): "Muy saludable" | "Equilibrada" | "Poco nutritiva" {
  return extraerCalificacion(raw || "");
}
...
export const analizarPatronAlimenticio = onCall<AnalizarPatronRequest>(
  ...
  async (request) => {
    try {
      ...
      const calificacionGeneral = extraerCalificacionGeneral(raw.calificacionGeneral ?? "");
